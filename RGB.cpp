...
extern const int MAX_COLOR_DEVIATION = 30;
extern const int MAX_X_DEVIATION = 7;
extern const int MAX_Y_DEVIATION = 7;
...
rect RGB::find_zone_edges(IplImage* image)
{
	...
	for (int y = 0; y < image->height; y++)
	{
		...
		{	
			if (abs(ptr[3*x] - B) < MAX_COLOR_DEVIATION && abs(ptr[3*x+1] - G) < MAX_COLOR_DEVIATION && abs(ptr[3*x + 2] - R) < MAX_COLOR_DEVIATION) k++;		
			else k = 0;
			if (k > ROW)
			{
				savePoint[t].x = x - ROW;
...
//-------------------------------------------------------------------------------------------------------------------------------
	...
	point LeftPoints[image->height*image->width];	//Массив для сохранения найденных точек левых границ объектов
	point RightPoints[image->height*image->width];	// --//-- правых границ
	...
	{												//находится в левом верхнем углу
		...
		for (x = 0; x < image->width; x++)
		{
			...
			{										//Если на фоне, то нужно найти объект (его левую границу) --> SEEK_OBJECT
													//Если на объекте, то нужно найти фон (правую границу объекта) --> SEEK_BACKGROUND
				if (abs(ptr[3*x] - B) > MAX_COLOR_DEVIATION || abs(ptr[3*x+1] - G) > MAX_COLOR_DEVIATION || abs(ptr[3*x + 2] - R) > MAX_COLOR_DEVIATION)
...
//-------------------------------------------------------------------------------------------------------------------------------
	point leftSort[t][(image->height)];				//Массивы для сохранения близкорасположенных точек. Первый индекс - номер 
	point rightSort[t][(image->height)];			//стикера, второй - его точки. Точке левой границы соответсвует точка правой
													//границы
													
	point testPoint;								//Тестовая точка, с которой будут сравниваться другие точки; по ходу цикла 
	testPoint.x = DELETE;							//будет меняться. Значение DELETE означает, что точка свободна
	...
	{
		while (testPoint.x == DELETE)				//Если тестовая точка < 0 ==> это либо первая итерация, либо прошел цикл 
...
//-------------------------------------------------------------------------------------------------------------------------------
	for (j = 0; j < z; j++)
	{
		...
		{
			if ((leftSort[j][i].x == 0 && leftSort[j][i].y == 0) || (rightSort[j][i].x == 0 && rightSort[j][i].y == 0))
			{
